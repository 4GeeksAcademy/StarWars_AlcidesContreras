...
export const Navbar = () => {
	...
	let infoDataPeople = store.dataPeople
	const [favoriteList, setFavoriteList] = useState([])

	console.log(infoDataPeople);
	

	useEffect(()=>{
		setFavoriteList(infoDataPeople);
	},[infoDataPeople]);
		...
		<>
			...
			<nav className="navbar navbar-expand-lg">
				<div className="container-fluid">
					<Link to="/">
						<span className="logostw"><svg xmlns="http://www.w3.org/2000/svg" x="10px" y="0px" width="200" height="200" viewBox="0 0 48 48">
					...
					<div className="collapse navbar-collapse fw-bold justify-content-center">
						<ul className="navbar-nav">
							<li className="nav-item dropdown">
								<a className="nav-link dropdown-toggle bg-warning" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
									Favorite <span>{favoriteList.length}</span>
								</a>
								<ul className="dropdown-menu">
									{favoriteList.length !== 0 ? 
										favoriteList.map((personaje, index)=>{
											return(
												<li className="" key={index}>
													<aside className="d-flex justify-content-between">
														<a className="dropdown-item p-1" >{personaje}</a>
														<span className="p-1" onClick={()=>actions.deleteFavorite(index)}><i className="fa-solid fa-trash"></i></span>
													</aside>
												</li>
											)
										}) 
